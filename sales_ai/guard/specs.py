...
SPECS: dict[str, ReadSpec] = {
	...
	# Readable because they are cancellable. `guard.documents` lets the agent submit and
	# cancel both, and an agent that can reverse an invoice but cannot read one back is the
	# wrong way round: the approval card would have nothing to show, and the model would be
	# asked to confirm a document it has never seen. Reading is the smaller power of the two.
	"Sales Invoice": ReadSpec(
		purpose="A bill raised against a customer. Read-only here; it is raised in ERPNext.",
		list_fields=(
			"name",
			"customer",
			"customer_name",
	...
	"Delivery Note": ReadSpec(
		purpose="A record of goods sent to a customer. Read-only here; it is raised in ERPNext.",
		list_fields=(
			"name",
			"customer",
			"customer_name",
		...
		search_fields=("name", "customer", "customer_name", "po_no", "lr_no"),
		filter_fields=(
			"status",
			"docstatus",
			"customer",
		...
		untrusted=frozenset(
			{"customer_name", "po_no", "lr_no", "transporter_name", "contact_email", "contact_mobile"}
		),
		children={
			"items": (
				"item_code",
	...
	"Item Price": ReadSpec(
		purpose="The selling or buying price of an item on a specific price list.",
		list_fields=(
			"name",
			"item_code",
			"item_name",
		...
		search_fields=("name", "item_code", "item_name", "price_list"),
		filter_fields=(
			"item_code",
			"price_list",
			"selling",
...
# Deliberately absent: Quotation, Sales Order, Sales Invoice and Delivery Note. They are
# submittable documents whose totals come out of ERPNext's pricing and tax engine, so they
# need a build-and-preview step of their own rather than a field-by-field setter. What the
# agent may do with them lives in `guard.quotations` and `guard.documents`.
#
...
WRITE_SPECS: dict[str, WriteSpec] = {
	...
	"Item": WriteSpec(
		...
		required=("item_code", "item_group"),
		identity="item_code",
	),
	"Item Price": WriteSpec(
		label="item price",
		create_fields=(
			"item_code",
			"price_list",
